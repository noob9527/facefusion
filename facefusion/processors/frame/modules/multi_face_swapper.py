...
from facefusion.multi_face_reference import get_face_references, set_face_references
...
def apply_args(program : ArgumentParser) -> None:
	...
	# frame_processors_globals.face_swapper_source_array = args.source_path_array
...
def process_frames(source_path : str, temp_frame_paths : List[str], update_progress : Update_Process) -> None:
	...
	source_face_list = [get_one_face(read_static_image(source_path)) for source_path in source_path_list]
...
def conditional_set_face_reference(temp_frame_paths : List[str]) -> None:
	source_path_list = facefusion.globals.source_path.split(',')
	if 'reference' in facefusion.globals.face_recognition and not get_face_references():
		reference_frame = read_static_image(temp_frame_paths[facefusion.globals.reference_frame_number])
		reference_faces = [get_one_face(reference_frame, index) for index in range(0, len(source_path_list))]
		set_face_references(reference_faces)
